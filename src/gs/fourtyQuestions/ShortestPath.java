 ...
 *
 *      1King's Cross-- 2St Pancras --- 3Angel ---- 4Old Street
 *      |                   \                            |
 *      |                    \                            |
 *      |                     \                            |
 *      Russell Square         3Farringdon --- 4Barbican6 --- 5Moorgate
 *      |                                                  /
 *      |                                                 /
 *      |                                                /
...
/**
    ...
    private static class TrainMap {
        ...
        public List<Station> shortestPath(String from, String to) {
            ...
            visited.add(from);
            while (!pq.isEmpty()){
                Pair<Integer,String> pr = pq.poll();
                Integer i = pr.getKey();
                String s = pr.getValue();
                Station currentStation = stations.get(s);
                currentStation.getNeighbours().stream().forEach(cp->{
                    if (!visited.contains(cp.getName())){
                        pq.add(new Pair<>(i+1,cp.getName()));
                        visited.add(cp.getName());
                    }
                });
            }
